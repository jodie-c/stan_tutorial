...
functions {
  ...
  vector coral_model(real t, vector y, vector theta_pars) {
    ...
    real beta = theta_pars[2];   // Mortality rate
    real gamma = theta_pars[3];  // Recovery rate
    real mu = theta_pars[4];     // Bare loss rate
...
generated quantities {
  ...
  // Sample from Student-t priors
  for (p in 1:4) {
    mu_pop[p] = student_t_rng(3, 1, 2);
    tau_pop[p] = fmax(1e-6, student_t_rng(3, 0, 1));
    tau_site[p] = fmax(1e-6, student_t_rng(3, 0, 1));
  }

  L_Omega_pop = lkj_corr_cholesky_rng(4, 2.0);
  L_Omega_site = lkj_corr_cholesky_rng(4, 2.0);
  ...
  for (s in 1:N_sites) {
    ...
    sigma[s] = fmax(1e-6, student_t_rng(3, 0, 1));
