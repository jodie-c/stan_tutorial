...
generated quantities {
  ...
  // Sample from wide normal priors
  for (p in 1:4) {
    mu_pop[p] = normal_rng(1, 5);
    tau_pop[p] = fmax(1e-6, normal_rng(0, 5));
    tau_site[p] = fmax(1e-6, normal_rng(0, 5));
  }
  ...
  for (s in 1:N_sites) {
    ...
    sigma[s] = fmax(1e-6, normal_rng(0, 3));
